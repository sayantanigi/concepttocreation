                            ...
                            foreach ($courses as $key => $value) {
                                ...
                                if(!empty($moduleData)) {
                                    foreach ($moduleData as $keyn => $item) {
                                        ...
                                        // echo "<br> Module = ".$item->id." Tot Mat ".$totalmaterial." attempt module = ".@$attemptModuleRow->attemptModule." Completed = ".$totalComModule;
                                    }
                                }
                                // Get total Course completed module.
                                /* $getCompletedModuleSql = "SELECT COUNT(DISTINCT `module`) as completedModule FROM `course_enrollment_status` where `course_id` = '" . @$value->course_id . "';";
                                $completedModuleRow = $this->db->query($getCompletedModuleSql)->row();
                                $totalCompletedModule = @$completedModuleRow->completedModule; */
